...
public class ContactsDBHints
{
    ...
    public static void printArrays()
    {
        ...
        {
            System.out.printf("%2d: %-10s: %d\n", i, contactNames[i], phoneNumbers[i]);
        }
    }
    public static boolean isFull()
    {   return entries == contactNames.length;
    }
    public static void addEntry(String name, long number)
    {
        ...
        contactNames[entries] = name;
        phoneNumbers[entries] = number;
        entries++;
    }
    ...
    {   if( !isSorted() )
        ...
        int low  = 0;
        int high = phoneNumbers.length - 1;
        int mid  = 0;
        while (low <= high)
        {   mid = (low + high) / 2;
            if (phoneNumbers[mid] == number)
            {   return contactNames[mid];
            } else if (phoneNumbers[mid] < number)
            {	low  = mid + 1;
            } else
            {	high = mid - 1;
            }
